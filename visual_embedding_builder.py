#!/usr/bin/env python3
"""
Build GR1 NanoLLaVA Dense LV-Embeddings Dataset (No Frames)
...
per-frame dense hidden states [L, H] from NanoLLaVA conditioned on the episode
...
def setup_logger(out_root: Path, verbose: bool = True) -> logging.Logger:
    ...
    logger = logging.getLogger("build_gr1_nanollava_embeddings")
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--hf-token", type=str, default=None, help="Hugging Face token (provide via argument or HF_TOKEN env var)")
        ...
        for episode_idx, vid_path in enumerate(tqdm(video_paths if args.limit is None else video_paths[: max(0, args.limit - episodes_processed)], desc=f"{task_dir.name} episodes")):
            ...
            for i in tqdm(range(0, len(frame_idxs), args.batch_size), desc=f"frames {video_relpath}", leave=False):
                ...
                try:
                    ...
                    # Extra validation: log input tensor devices/dtypes
                    try:
                        def _info(name, x):
                            try:
                                import torch  # type: ignore
                                if isinstance(x, torch.Tensor):
                                    debug_log(f"Input {name}: device={x.device}, dtype={x.dtype}, shape={tuple(x.shape)}")
                                elif isinstance(x, (list, tuple)) and x and isinstance(x[0], torch.Tensor):
                                    ts = x[0]
                                    debug_log(f"Input {name}[0]: device={ts.device}, dtype={ts.dtype}, shape={tuple(ts.shape)}")
                            except Exception:
                                pass
                        _info("input_ids", inputs.get("input_ids"))
                        _info("attention_mask", inputs.get("attention_mask"))
                        _info("images", inputs.get("images"))
                    except Exception:
                        pass
                except Exception as e:
                ...
                with torch.no_grad():
                    try:
                        # Ensure all critical submodules and inputs are on the same device before forward
                        try:
                            model_param_device = next(model.parameters()).device
                        except Exception:
                            model_param_device = device
                        # Align vision tower and mm_projector to model device (best-effort)
                        with contextlib.suppress(Exception):
                            base_align = getattr(model, "get_model", lambda: model)()
                            try:
                                setattr(base_align, "device", model_param_device)
                            except Exception:
                                pass
                            vt_align = None
                            if hasattr(base_align, "get_vision_tower"):
                                vt_align = base_align.get_vision_tower()
                            elif hasattr(base_align, "vision_tower"):
                                vt_align = getattr(base_align, "vision_tower")
                            if vt_align is not None and hasattr(vt_align, "to"):
                                vt_align.to(device=model_param_device, dtype=dtype_torch)
                            mm_proj_align = getattr(base_align, "mm_projector", None) or getattr(model, "mm_projector", None)
                            if mm_proj_align is not None:
                                for mod in mm_proj_align.modules():
                                    if hasattr(mod, "to"):
                                        mod.to(device=model_param_device)
                        # Align inputs to model device
                        imgs = inputs.get("images", None)
                        if imgs is not None:
                            if hasattr(imgs, "to"):
                                inputs["images"] = imgs.to(device=model_param_device, dtype=dtype_torch)
                            elif isinstance(imgs, (list, tuple)) and len(imgs) > 0 and hasattr(imgs[0], "to"):
                                inputs["images"] = type(imgs)([t.to(device=model_param_device, dtype=dtype_torch) for t in imgs])  # type: ignore
                        if "input_ids" in inputs and hasattr(inputs["input_ids"], "to"):
                            inputs["input_ids"] = inputs["input_ids"].to(device=model_param_device)
                        if "attention_mask" in inputs and inputs["attention_mask"] is not None and hasattr(inputs["attention_mask"], "to"):
                            inputs["attention_mask"] = inputs["attention_mask"].to(device=model_param_device)
                        debug_log(f"Pre-forward devices aligned to {model_param_device}")
                        ...
                        if "Expected all tensors to be on the same device" in msg:
                            try:
                                ...
                                if mm_proj is not None:
                                    # Align mm_projector device with the active compute device (prefer GPU)
                                    for mod in mm_proj.modules():
                                        if hasattr(mod, "to"):
                                            mod.to(device=device)
                                    # Ensure inputs match model param device after realignment
                                    try:
                                        model_param_device = next(model.parameters()).device
                                    except Exception:
                                        model_param_device = device
                ...
                # last: [B, L, H]
                # Slice sequence to keep only the vision tokens (drop text tokens)
                try:
                    # Infer number of image tokens V for this forward pass
                    if args.assume_image_tokens is not None:
                        v_tokens = int(args.assume_image_tokens)
                    else:
                        # Use tokenizer length of the joined prompt (text tokens per sample)
                        n_text_tokens_batch = int(tokenizer_len_fn(joined_prompt))
                        v_tokens = int(last.size(1)) - n_text_tokens_batch
                    if v_tokens <= 0:
                        raise ValueError(f"Non-positive inferred V={v_tokens} (L={int(last.size(1))})")
                    # Keep only the first V tokens along the sequence (vision tokens precede text)
                    if v_tokens < int(last.size(1)):
                        last = last[:, :v_tokens, :]
                    debug_log(f"Sliced to vision tokens only: V={int(last.size(1))}")
                except Exception as e:
                    debug_log(f"ERROR slicing to vision tokens: {e}", "ERROR")
                    logger.error("Failed to slice vision tokens for %s: %s", video_relpath, e)
                    write_jsonl(failures_path, [{
                        "video_relpath": video_relpath,
                        "episode_index": ep_idx,
                        "error": f"slice_vision: {type(e).__name__}: {e}",
                    }])
                    failures += 1
                    episodes_processed += 1
                    continue
                ...
                # Override representative attention mask to ones of length V (vision-only)
                if rep_attention_mask is None or rep_attention_mask.shape[0] != L:
                    rep_attention_mask = np.ones((L,), dtype=np.uint8)
            ...
            # Token counts
            # After slicing, the sequence contains only vision tokens
            t_img = int(L)
            t_text = int(0)
