...
class CrossAttention(nn.Module):
    def __init__(self, hidden_size, head_dim, num_heads, num_key_value_heads, causal=False):
        ...
        self.num_heads = num_heads
        self.num_key_value_heads = num_key_value_heads
        self.causal = causal  # for cross-attn keep False
    ...
    def forward(self, cos_sin: CosSin, hidden_states: torch.Tensor, encoder_states: torch.Tensor) -> torch.Tensor:
        B, S, _ = hidden_states.shape
        S_enc = encoder_states.shape[1]

        q = self.q_proj(hidden_states).view(B, S, self.num_heads, self.head_dim)
        kv = self.kv_proj(encoder_states).view(B, S_enc, 2 * self.num_key_value_heads, self.head_dim)
        k = kv[:, :, :self.num_key_value_heads]
        v = kv[:, :, self.num_key_value_heads:]

        # No RoPE for cross-attn by default
        q, k, v = map(lambda t: einops.rearrange(t, 'B S H D -> B H S D'), (q, k, v))
        attn_output = scaled_dot_product_attention(query=q, key=k, value=v, is_causal=False)
        attn_output = einops.rearrange(attn_output, 'B H S D -> B S H D').reshape(B, S, self.output_size)
        return self.o_proj(attn_output)